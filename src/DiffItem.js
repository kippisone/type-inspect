...
class DiffItem {
  ...
  handleArray (leftValue, rightValue) {
    ...
    for (let i = 0; i < len; i++) {
      const left = leftValue ? leftValue[i] : { type: 'undefined', kind: 'undefined', value: undefined }
      const right = rightValue ? rightValue[i] : { type: 'undefined', kind: 'undefined', value: undefined }
      const diffItem = new DiffItem(left, right)
